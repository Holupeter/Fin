...
export default function EditPotModal({ isOpen, onClose, pot }: EditPotModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex justify-center items-center bg-grey-900/40 backdrop-blur-sm px-4">
      <div className={`flex flex-col items-start p-8 gap-5 w-full max-w-[335px] md:max-w-[560px] h-auto max-h-[90vh] md:h-[491px] bg-white rounded-xl relative shadow-xl ${isThemeOpen ? 'overflow-visible' : 'overflow-y-auto overflow-x-hidden'} transition-all duration-300 no-scrollbar`}>
        {/* Header */}
        <div className="flex flex-row justify-between items-center w-full">
          <h2 className="text-preset-2 md:text-preset-1 text-grey-900">Edit Pot</h2>
          <button
            onClick={onClose}
        ...
        <p className="text-preset-4 text-grey-500 w-full">
          If your savings targets change, you can update your pot. These changes will take effect immediately.
        </p>
          ...
          <div className="flex flex-col items-start gap-1 w-full relative">
            ...
            >
              <div className="flex flex-row items-center gap-3">
                <div className={`w-4 h-4 rounded-full ${themes.find(t => t.name.toLowerCase() === themeColor.toLowerCase())?.color || 'bg-green'} flex-shrink-0`}></div>
                  ...
                  <React.Fragment key={theme.name}>
                    ...
                    >
                      <div className="flex flex-row items-center gap-3">
                        ...
                        <span className={`text-preset-4 transition-all ${themeColor.toLowerCase() === theme.name.toLowerCase() ? 'text-grey-900 font-bold' : 'text-grey-900 group-hover:font-bold'}`}>
                          {theme.name}
                        </span>
                      </div>
                      {themeColor.toLowerCase() === theme.name.toLowerCase() && (
